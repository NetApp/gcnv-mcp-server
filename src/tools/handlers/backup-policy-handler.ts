...
interface DeleteBackupPolicyParams {
    projectId: string;
    location: string;
    backupPolicyId: string;
    force?: boolean;
...
interface GetBackupPolicyParams {
    projectId: string;
    location: string;
    backupPolicyId: string;
}

interface ListBackupPoliciesParams {
    projectId: string;
    location: string;
    filter?: string;
    pageSize?: number;
...
interface UpdateBackupPolicyParams {
    projectId: string;
    location: string;
    backupPolicyId: string;
    dailyBackupLimit?: number;
 ...
 */
export const createBackupPolicyHandler: ToolHandler = async (args, extra) => {
    try {
 ...
 */
export const deleteBackupPolicyHandler: ToolHandler = async (args, extra) => {
    try {
        const { projectId, location, backupPolicyId, force } = args;
 ...
 */
export const getBackupPolicyHandler: ToolHandler = async (args, extra) => {
    try {
        ...
        const result = {
            ...
            createTime: backupPolicy.createTime ? new Date(Number((backupPolicy.createTime as any).seconds || 0) * 1000) : new Date(),
        ...
        return {
            content: [{ 
                type: "text", 
                text: `Retrieved backup policy: ${result.name}`
            }],
            structuredContent: result
        };
    } catch (error: any) {
 ...
 */
export const listBackupPoliciesHandler: ToolHandler = async (args, extra) => {
    try {
        ...
        // Handle API response
        const backupPolicies = Array.isArray(response) ? response : (response as any).backupPolicies || [];
        const nextPageToken = typeof response === 'object' && response !== null ? (response as any).nextPageToken || '' : '';
        ...
        const result = {
            backupPolicies: backupPolicies.map((policy: any) => ({
                ...
                createTime: policy.createTime ? new Date(Number((policy.createTime as any).seconds || 0) * 1000) : undefined,
        ...
        return {
            content: [{ 
                type: "text", 
                text: `Listed ${result.backupPolicies.length} backup policies.`
            }],
            structuredContent: result
        };
    } catch (error: any) {
 ...
 */
export const updateBackupPolicyHandler: ToolHandler = async (args, extra) => {
    try {
        const { projectId, location, backupPolicyId, dailyBackupLimit, weeklyBackupLimit, monthlyBackupLimit, description, enabled, labels } = args;
