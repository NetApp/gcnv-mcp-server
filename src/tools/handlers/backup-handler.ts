...
function formatBackupData(backup: any): any {
    ...
    if (backup.enforcedRetentionEndTime) result.enforcedRetentionEndTime = backup.enforcedRetentionEndTime;
...
export const createBackupHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
            ...
            console.log("Create backup request:", request);
            ...
            console.log("Backup creation operation started:", operationName);
...
export const deleteBackupHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
...
// Get Backup Handler
export const getBackupHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
            const { projectId, location, backupVaultId, backupId } = args;
            ...
            console.log("Raw backup data:", backup);

            // Format the backup data
            const formattedData = formatBackupData(backup);
            
            console.log("Formatted backup data:", formattedData);
...
export const listBackupsHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
            ...
            const [backups, _ ,nextPageToken] = await netAppClient.listBackups(options);
            console.log("Raw backups data:", backups);

            let formattedBackups = backups.map((backup: any) => formatBackupData(backup));
            console.log("Formatted backups data:", formattedBackups);
...
export const restoreBackupHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
            const { 
                projectId, 
            ...
            console.log("Available methods:", Object.keys(netAppClient).filter(k => typeof netAppClient[k as keyof typeof netAppClient] === 'function'));
            ...
            try {
                ...
                console.error("Error in restore operation:", restoreError);
                throw restoreError;
            }
