...
function formatBackupVaultData(backupVault: any): any {
    ...
    if (backupVault.destinationBackupVault) result.destinationBackupVault = backupVault.destinationBackupVault;
...
export const createBackupVaultHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
            ...
            return {
                content: [{
                    type: "text" as const,
                    text: `Backup vault creation initiated. Operation ID: ${operation}`
                }],
                structuredContent: {
...
export const deleteBackupVaultHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
            const { projectId, location, backupVaultId, force } = args;
            ...
            return {
                content: [{
                    type: "text" as const,
                    text: `Backup vault deletion initiated. Operation ID: ${operation}`
                }],
                structuredContent: {
                    success: true,
            ...
            console.error("Error deleting backup vault:", error);
            ...
            if (error.code === 5) { // NOT_FOUND
                ...
                errorMessage = "Failed precondition. The backup vault may have dependent backups that need to be deleted first.";
            }
...
export const getBackupVaultHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
            ...
            console.error("Error getting backup vault:", error);
...
export const listBackupVaultsHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
...
export const updateBackupVaultHandler: ToolHandler = 
    async (args: { [key: string]: any }, extra: any) => {
        try {
            ...
            return {
                content: [{
                    type: "text" as const,
                    text: `Backup vault update initiated. Operation ID: ${operation}`
                }],
                structuredContent: {
                    name,
                    operationId: operation.name
            ...
            console.error("Error updating backup vault:", error);
