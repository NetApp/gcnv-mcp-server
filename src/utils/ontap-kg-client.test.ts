...
describe('ontap-kg-client discoverViaKg', () => {
  ...
  it('returns null when categories or endpoints arrays contain invalid items', async () => {
    process.env.ONTAP_KG_URL = 'https://kg.example/';
    vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce({
        ok: true,
    ...
    const { discoverViaKg } = await import('./ontap-kg-client.js');
    const categoriesResult = await discoverViaKg({
      schemaVersion: 'ontap-kg/1',
      kind: 'categories',
    });
    const searchResult = await discoverViaKg({
      schemaVersion: 'ontap-kg/1',
      kind: 'search',
      search: 'volumes',
    });

    expect(categoriesResult).toBeNull();
